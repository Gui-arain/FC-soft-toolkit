/*
  uORB driver for the ICM-40609-D IMU designed for 32kHz FIFO SPI burst
  reading. Registers two independent uORB topics (accel, gyro) off one
  physical chip -- see icm40609d_uorb.h for the registration entry point.
*/
 ...
 *
 * Bank 0/1/2/4 addresses below are register-identical to the ICM-42688-P
 * (confirmed against the ICM-40609-D datasheet, resources/datasheets/IMU/
 * ICM-40609-D-v1.2.pdf, §12.1-12.3) -- only WHO_AM_I and the ACCEL_FS_SEL
 * encoding differ between the two chips (see icm_set_accel_fsr() and
 * g_icm_accel_scale[] below).
 */
...
enum icm_regaddr_e
{
   ...
   *   111 = ±15.625 dps
   *
   * True POR default is 000 (±2000dps) -- icm_reset() programs this
   * explicitly rather than relying on it (see icm_reset() comments).
   *
   * GYRO_ODR (bits 3:0):
   *   0001 = 32 kHz  … 0110 = 1 kHz … 1111 = 500 Hz
   *   True POR default is 0111 (200 Hz) -- icm_reset() reprograms to 1kHz.
   */

  GYRO_CONFIG0 = 0x4f,                     /* Reset: 0x07 */
   ...
   *
   * ACCEL_FS_SEL (bits 7:5) -- NOTE: this chip's encoding differs from
   * the ICM-42688-P's (no ±2g range, ±32g is sel 0 not sel 4):
   *   000 = ±32 g  (POR default)
   *   001 = ±16 g
   *   010 = ±8  g
   *   011 = ±4  g
   *   100-111 = Reserved
   *
   * icm_reset() explicitly reprograms this to ±16g rather than relying
   * on the ±32g POR default -- see icm_reset() comments.
   *
   * ACCEL_ODR (bits 3:0):
   *   0001 = 32 kHz … 0110 = 1 kHz … 1111 = 500 Hz
   *   True POR default is 0111 (200 Hz) -- icm_reset() reprograms to 1kHz.
   ...
   * Register-identical to the ICM-42688-P for this whole block. */
  ...
  /* Gyro notch-filter bandwidth: its own register (STATIC10),
   * field is bits[6:4].
  ...
  /* ======================= Bank 2 ============================= *
   * The accel static config lives in Bank 2, register-identical to
   * the ICM-42688-P. Switch to bank 2 for these. */
...
/* Accel raw-LSB-to-physical-unit scale factors, indexed by ACCEL_FS_SEL.
 *
 * NOTE: this chip's FS_SEL encoding is NOT the same ordering as the
 * ICM-42688-P's -- sel 0 is +-32g here (POR default), there is no +-2g
 * range at all, and icm_set_accel_fsr() below maps input "g" values to
 * these selectors accordingly. Getting this table's ordering wrong
 * silently misreports acceleration by up to 2x with a config write that
 * still "succeeds".
 */

static const float g_icm_accel_scale[] =   /* m/s^2 per LSB */
{
  32.0f * 9.80665f / 32768.0f,  /* sel 0: +-32g */
  16.0f * 9.80665f / 32768.0f,  /* sel 1: +-16g */
  8.0f  * 9.80665f / 32768.0f,  /* sel 2: +-8g  */
  4.0f  * 9.80665f / 32768.0f,  /* sel 3: +-4g  */
};

/* Gyro raw-LSB-to-physical-unit scale factors, indexed by GYRO_FS_SEL.
 * Same FS_SEL ordering/values as the ICM-42688-P.
 */
...
static int __icm_read_reg_spi(FAR struct icm_dev_s *dev, enum icm_regaddr_e reg_addr, FAR uint8_t *buf, uint16_t len)
{
  ...
  /* Grab and configure the SPI master device: always mode 3, 10MHz. */
 ...
 * Explicitly programs gyro/accel FSR+ODR (this chip's POR default is
 * +-32g/+-2000dps @ 200Hz, not the +-16g/+-2000dps @ 1kHz this driver
 * actually wants)
...
static int icm_reset(FAR struct icm_dev_s *dev)
{
   ...
   * INTF_CONFIG0__FIFO_COUNT_ENDIAN = 1        => big-endian (this is also the
   *                                                POR default on this chip):
   *                                                __icm_read_fifo_count() combines
   *                                                FIFO_COUNTH/L as (high<<8)|low,
   *                                                and this MUST match what's actually
   *                                                on the wire or the 16-bit count comes
   *                                                back byte-swapped (e.g. a real count
   *                                                of 0x0080 misread as 0x8000 = 32768).
  ...
  /* GYRO_CONFIG0 / ACCEL_CONFIG0: this chip's true POR default is
   * +-32g/+-2000dps @ 200Hz (FS_SEL=000, ODR=0111) -- program +-2000dps
   * gyro / +-16g accel (ACCEL_FS_SEL=001, see icm_set_accel_fsr()'s
   * comment on this chip's FSR encoding) at 1kHz (ODR_SEL=0110)
   * explicitly rather than relying on the chip defaults.
   */
  const uint8_t gyro_config0 = (uint8_t)(0 << GYRO_CONFIG0__GYRO_FS_SEL__SHIFT) |
                               (0x06 << GYRO_CONFIG0__GYRO_ODR__SHIFT);
  const uint8_t accel_config0 = (uint8_t)(1 << ACCEL_CONFIG0__ACCEL_FS_SEL__SHIFT) |
                                (0x06 << ACCEL_CONFIG0__ACCEL_ODR__SHIFT);

  /* INT_CONFIG: reset default is open-drain + active-low (0x00). Our board
   * wiring (GPIO_IMU_INT: floating input, rising-edge EXTI trigger)
   * assumes push-pull + active-high, so reconfigure INT1 to match.
   */
   ...
   * comes back wrong (0x00, 0xff, or anything but 0x3b), the chip isn't
  ...
  if (whoami != 0x3b)
    {
      snerr("ERROR: ICM-40609-D WHO_AM_I mismatch: got 0x%02x, expected 0x3b "
  ...
  ret = __icm_write_reg(dev, GYRO_CONFIG0, &gyro_config0, 1);
  if (ret < 0)
    {
      nxmutex_unlock(&dev->lock);
      return ret;
  ...
  ret = __icm_write_reg(dev, ACCEL_CONFIG0, &accel_config0, 1);
  if (ret < 0)
    {
      nxmutex_unlock(&dev->lock);
      return ret;
...
static void icm_fifo_worker(FAR void *arg)
{
  ...
  /* The datasheet states the FIFO_CONFIG2/3 watermark interrupt only
   * fires once -- rewriting the same watermark value re-arms it for
   * the next crossing.
   */

  icm_program_watermark(dev, dev->watermark_samples);
 ...
 * icm_set_accel_fsr()
 *
 * NOTE: the ICM-40609-D's ACCEL_FS_SEL encoding is NOT the same ordering
 * as the ICM-42688-P's -- this chip has no +-2g range at all, and +-32g
 * is selector 0 (POR default) rather than a special-cased selector 4.
 * See the ACCEL_CONFIG0 comment in the register enum above.
 ****************************************************************************/
...
static int icm_set_accel_fsr(FAR struct icm_dev_s *dev, uint16_t g)
{
  ...
  switch (g)
    {
      case 32: fsr_sel = 0; break;
      case 16: fsr_sel = 1; break;
      case 8:  fsr_sel = 2; break;
      case 4:  fsr_sel = 3; break;
      default: return -EINVAL;   /* includes 2g -- not supported on this chip */
    }
 ...
 *
 * Algorithm from the ICM-4x6xx datasheet:
 *
 ...
 *
 * Description:
 *   Registers the ICM-40609-D as a pair of uORB topics.
 *
 * Input Parameters:
...
int icm40609d_uorb_register(int devno, FAR struct icm_config_s *config)
{
  ...
  /* icm_reset() explicitly programs +-16g / +-2000dps / 1kHz (see its
   * comments on this chip's true POR default) -- match the cached scale
   * factors and sample rate to that until SET_*_FSR or set_interval()
   * change them.
   */

  priv->accel_scale      = g_icm_accel_scale[1];  /* sel 1: +-16g on this chip */
  ...
  priv->accel.lower.nbuffer = CONFIG_SENSORS_ICM40609D_UORB_ACCEL_ORB_BUFSIZE;

  ret = sensor_register(&priv->accel.lower, devno);
  if (ret < 0)
    {
      snerr("ERROR: Failed to register ICM-40609-D accelerometer topic: %d\n", ret);
      goto err_free;
    }

  priv->gyro.lower.type    = SENSOR_TYPE_GYROSCOPE;
  priv->gyro.lower.ops     = &g_icm_ops;
  priv->gyro.lower.nbuffer = CONFIG_SENSORS_ICM40609D_UORB_GYRO_ORB_BUFSIZE;

  ret = sensor_register(&priv->gyro.lower, devno);
  if (ret < 0)
    {
      snerr("ERROR: Failed to register ICM-40609-D gyroscope topic: %d\n", ret);
